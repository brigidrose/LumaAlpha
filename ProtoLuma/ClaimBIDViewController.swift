...
class ClaimBIDViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.submitButton.layer.borderColor = (UIApplication.sharedApplication().delegate as! AppDelegate).window?.tintColor.CGColor
        self.submitButton.layer.cornerRadius = 6
        self.submitButton.setTitle("Submit", forState: UIControlState.Normal)
        self.submitButton.setTitleColor((UIApplication.sharedApplication().delegate as! AppDelegate).window?.tintColor, forState: UIControlState.Normal)
        self.submitButton.clipsToBounds = true
        self.submitButton.setTitleColor(UIColor.whiteColor(), forState: UIControlState.Highlighted)
        self.submitButton.titleLabel?.textColor = UIColor.whiteColor()
        self.submitButton.titleLabel?.font = UIFont.systemFontOfSize(17, weight: UIFontWeightLight)
        self.submitButton.setBackgroundImage((UIApplication.sharedApplication().delegate as! AppDelegate)
        ...
        let verticalConstraints = NSLayoutConstraint.constraintsWithVisualFormat("V:|->=60-[instructionLabel(30)]-12-[BIDTextField(50)]-30-[submitButton(44)]->=0-|", options: [NSLayoutFormatOptions.AlignAllLeft, NSLayoutFormatOptions.AlignAllRight], metrics: nil, views: viewsDictionary)
